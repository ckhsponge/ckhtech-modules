locals {
  ...
  string_indexes = [
    for i in range(var.global_secondary_indexes_string_count) : {
      name           = "string_index${i}"
  ...
  string_index_attributes = flatten([
    for s in local.string_indexes : [
      {
        name = s.hash_key
  ...
  number_indexes = [
    for i in range(var.global_secondary_indexes_number_count) : {
      name           = "number_index${i}"
  ...
  number_index_attributes = flatten([
    for s in local.number_indexes : [
      {
        name = s.hash_key
        type = s.hash_key_type
      }, {
  ...
  attributes = flatten([
    local.hash_key_attribute, local.additional_attributes, local.string_index_attributes, local.number_index_attributes
  ])
  global_secondary_indexes = flatten([
    var.additional_global_secondary_indexes, local.string_indexes, local.number_indexes
  ])
}
...
data aws_iam_policy_document writer {
  ...
  statement {
    ...
    condition {
      test     = "Bool"
      values = ["true"]
      variable = "dynamodb:FullTableScan"
    }
  }
}
