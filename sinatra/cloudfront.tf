...
module "cloudfront" {
  ...
  service = var.service
  name = var.name
  origin_domain_name = replace(replace(aws_apigatewayv2_stage.lambda.invoke_url,"https://",""),"/","") #aws_apigatewayv2_domain_name.main.domain_name_configuration[0].target_domain_name
